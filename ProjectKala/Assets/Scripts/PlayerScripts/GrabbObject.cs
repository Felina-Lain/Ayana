...
public class GrabbObject : MonoBehaviour
{
    ...
    void Start()
    {
     ...
     //   m_objectToGrabb = this.transform;
    }
    ...
    void Update()
    {
        ...
        if (!m_isGrabbing && Input.GetButtonDown("Grabb"))
        {
            ...
            if (objectToGrabb.GetComponent<ObjectController>().makeStep)
            {
                ...
                StartCoroutine(Dragouille2(objectToGrabb));
            }
    ...
    private IEnumerator Dragouille2(Transform objectGrabbed)
    {
    ...
    private void CalculateProbeNormals()
    {
        ControlProbe cp = GetComponentInChildren<ControlProbe>();
        if (cp != null)
            cp.CalculateNormals();
    }
